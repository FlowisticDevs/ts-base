...
export const run: execute = async (client, interaction, args) => {
  ...
  embed.setTitle(
    `${capitalizeFirstLetterfromMultipleWords(category.toLowerCase())} Commands`
  );
  ...
  embed.addFields([
    {
      name: client.commandCategories[
        client.commandCategories.indexOf(category.toLowerCase())
      ],
      value: await client.getCommandsbyCategory(
