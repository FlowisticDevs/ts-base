...
export const slashoptions: ApplicationCommandOptionData[] = [
  ...
  },
  {
    name: "url",
    type: ApplicationCommandOptionType.String,
    description: "If you selected watching, enter a URL here",
    required: false,
  },
];
...
export const run: execute = async (client, interaction, args) => {
  ...
  if (args.get("type").value.toString() == "1") {
    if (!args.get("url")) {
      interaction.reply(
        "Since you selected streaming as the presence type, please enter a URL"
      );
      ...
      client.user.setPresence({
        ...
        activities: [
          {
            name: name,
            type: parseInt(args.get("type").value.toString()),
            url: args.get("url").value.toString(),
          },
        ],
      });
      datatosave = {
        ...
        activities: [
          {
            name: args.get("name").value.toString(),
            type: parseInt(args.get("type").value.toString()),
            url: args.get("url").value.toString(),
          },
        ],
    ...
    client.user.setPresence({
      status: args.get("status").value as PresenceStatusData,
      activities: [
        { name: name, type: parseInt(args.get("type").value.toString()) },
      ],
    });
    datatosave = {
      status: args.get("status").value as PresenceStatusData,
  ...
  interaction.reply("Successfully changed the presence status");
