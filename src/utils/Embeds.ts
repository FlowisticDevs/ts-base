 ...
 */
function Success(
  message: string = "Completed action successfully.",
  message2: string
): EmbedBuilder {
  return new EmbedBuilder()
    .setDescription(message)
...
function NoPerms(message: string, message2: string): EmbedBuilder {
  return new EmbedBuilder()
    .setTitle(message)
    .setDescription(`You need ${message2} permissions to use this command`)
    .setColor(colors.noPerms);
}
  ...
  switch (direction) {
    case "next":
          ...
          new ButtonBuilder()
            .setCustomId(
              `${command_name}-previous-${
                selected_amount + amount_per_page
              }-${extra_args}`
            )
            .setLabel(`⬅️`)
      ...
      if (selected_amount < result.response.length - amount_per_page) {
        for (
          let i = selected_amount + amount_per_page;
          i < selected_amount + amount_per_page * 2;
          i++
        ) {
        ...
        embed.setTitle(
          `${embed_title} (${
            (selected_amount + amount_per_page) / amount_per_page + 1
          })`
        );
        interaction.update({ embeds: [embed], components: [buttonrow] });
      } else {
        interaction.reply({
          embeds: [Error("Can't go further than that")],
          ephemeral: true,
        });
      }
      break;
    case "previous":
      ...
      if (selected_amount - amount_per_page < amount_per_page) {
        buttonrow2 = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId(
              `${command_name}-previous-${
                selected_amount - amount_per_page
...
export { Success, Error, Loading, NoPerms, handlePagination };
