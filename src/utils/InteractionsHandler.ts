...
function handleCommandFile(client: Bot, file: Command) {
  if (file.slashoptions) {
    if (!file.type) {
      client.slashdata.push({
        name: file.name,
        description: file.description,
        options: file.slashoptions,
        dmPermission: false,
      });
      if (file.aliases) {
        for (let i = 0; i < file.aliases.length; i++) {
          client.slashdata.push({
            name: file.aliases[i],
            description: `This is an alias for the ${file.name} command`,
            options: file.slashoptions,
            dmPermission: false,
          });
        }
...
export async function canUserExecuteCommand(
  command: Command | Button | Select_Menu | Modal,
  client: Bot,
  member: GuildMember
    ...
    if (command.permissions == "Owner") {
      if (member.id == client.owner) {
        ...
        return {
          ...
          embed: NoPerms(
            "Insufficient perms",
            "Only the owner of the bot can run this command"
          ),
        };
      }
    } else {
      ...
      if (invalidPerms.length > 0) {
        return {
          allowed: false,
          embed: NoPerms("Insufficient perms", `${invalidPerms.join(", ")}`),
        };
      } else {
        return { allowed: true, embed: null };
      }
