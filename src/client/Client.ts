...
import {
  ...
  handleSelectMenus,
  handleModals,
} from "../utils/InteractionsHandler";
...
class Bot extends Client {
  ...
  public async start(): Promise<void> {
    ...
    command_files.map(async (value: string) => {
      ...
      console.log(
        `[${hours}:${minutes}:${seconds}] Loaded command: ${file.name}`
      );
    });
    ...
    button_files.map(async (value: string) => {
      ...
      console.log(
        `[${hours}:${minutes}:${seconds}] Loaded button: ${file.name}`
      );
    });
    ...
    modal_files.map(async (value: string) => {
      ...
      console.log(
        `[${hours}:${minutes}:${seconds}] Loaded modal: ${file.name}`
      );
    });
    ...
    event_files.map(async (value: string) => {
      ...
      console.log(`[${hours}:${minutes}:${seconds}] Hooked: ${file.name}`);
    });
    this.getCommandsbyCategory = async function getCommandsbyCategory(
      category: string
    ): Promise<string> {
