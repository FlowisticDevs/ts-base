  ...
  switch (interaction.type) {
    case InteractionType.ApplicationCommand:
      if (interaction.isContextMenuCommand()) {
        ...
        let result = await canUserExecuteCommand(
          command,
          client,
        ...
        if (time_stamps.has(interaction.user.id)) {
          ...
          if (current_time < expiration_time) {
            ...
            interaction.reply({
              embeds: [
                Error(
                  `Please wait ${time_left.toFixed(1)}s before you can use ${
                    command.name
                  } again`
                ),
              ],
        ...
        const time = moment().format("HH:mm:ss");
        console.log(
          `[${time}] ${interaction.user.tag} ran /${interaction.commandName} in guild: ${interaction.guild.name}`
        );
        const command: Command =
          client.commands.get(interaction.commandName) ||
          client.commands.find(
    ...
    case InteractionType.MessageComponent:
      if (interaction.isButton()) {
        console.log(
          `[${time}] ${interaction.user.tag} ran /${interaction.customId} in guild: ${interaction.guild.name}`
        );
        ...
        const button: Button = buttonwithargs
        ...
        console.log(
          `[${time}] ${interaction.user.tag} selected ${interaction.customId} in guild: ${interaction.guild.name}`
        );
        ...
        const select_menu: Select_Menu = select_menuwithargs
          ? client.select_menus.get(interaction.customId.split("-")[0])
          : client.select_menus.get(interaction.customId);
        ...
        try {
          await select_menu.run(
            client,
            interaction,
            interaction.values.toString().split("-")
          );
        } catch (reason) {
          console.error(reason);
          if (!interaction.replied) {
    ...
    case InteractionType.ModalSubmit:
      console.log(
        `[${time}] ${interaction.user.tag} submitted ${interaction.customId} in guild: ${interaction.guild.name}`
      );
      ...
      const modal: Modal = modalwithargs
